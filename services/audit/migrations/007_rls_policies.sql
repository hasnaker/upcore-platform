...
CREATE POLICY IF NOT EXISTS tenant_isolation_audit_events
    ON audit_events
    USING (tenant_id = current_setting('app.tenant_id', true)::UUID);

CREATE POLICY IF NOT EXISTS tenant_isolation_kvkk_access_log
    ON kvkk_access_log
    USING (tenant_id = current_setting('app.tenant_id', true)::UUID);

CREATE POLICY IF NOT EXISTS tenant_isolation_dsr_requests
    ON kvkk_dsr_requests
    USING (tenant_id = current_setting('app.tenant_id', true)::UUID);

CREATE POLICY IF NOT EXISTS tenant_isolation_exports
    ON audit_exports
    USING (tenant_id = current_setting('app.tenant_id', true)::UUID);
