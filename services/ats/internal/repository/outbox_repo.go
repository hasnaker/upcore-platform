...
func (r *outboxAdminRepo) beginTenantTx(ctx context.Context, tenantID uuid.UUID, readOnly bool) (*sqlx.Tx, error) {
...
func (r *outboxAdminRepo) Stats(ctx context.Context, tenantID uuid.UUID, maxRetries int) (*OutboxStats, error) {
	tx, err := r.beginTenantTx(ctx, tenantID, true)
	if err != nil {
		return nil, err
	}
...
func (r *outboxAdminRepo) Replay(ctx context.Context, tenantID, id uuid.UUID) error {
	tx, err := r.beginTenantTx(ctx, tenantID, false)
	if err != nil {
		return err
	}
	defer func() { _ = tx.Rollback() }()
