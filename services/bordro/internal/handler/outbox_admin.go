...
// NewOutboxAdminHandler constructs the handler. maxRetries should match
// OutboxDispatcher.MaxRetries so DLQ reporting is consistent.
...
// Stats handles GET /outbox/stats.
...
// ListDLQ handles GET /outbox/dlq — events that exceeded max_retries.
...
// Replay handles POST /outbox/{id}/replay — reset attempts to 0 so the
// dispatcher picks the row up again on its next tick.
