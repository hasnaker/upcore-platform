...
// Connect opens a sqlx connection pool to PostgreSQL and verifies connectivity.
func Connect(dsn string) (*sqlx.DB, error) {
	return ConnectWithOpts(dsn, 25, 5, 5*time.Minute)
}
...
// SetTenantRLS sets the current transaction's `app.tenant_id` GUC so that
// row-level security policies key off of the caller's tenant.
...
// SetTenantRLSOnConn sets tenant GUC on a raw sql.Conn (used for non-tx operations).
